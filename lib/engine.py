...
def look_clip():
    ...
    clip = clip_lib("guitar_a3")
    #clip = clip_lib("saxophone_a3")
    #clip = clip_lib("clarinet_a3")

    # computing these also graphs related information
    #clip.get_envelope()
    #clip.get_harmonics()

    clip.padded(0.5,0).play()
...
def look_synth():
    ...
    synth = synth_lib("guitar")
    #synth = synth_lib("clarinet")
    #synth = synth_lib("saxophone")

    duration = 1
    freq2 = 5/4 * synth.base_fundamental
    synth_clip = synth(duration=duration)
    synth_clip2 = synth(freq2, duration)
    ...
    (
        ...
        synth_clip2 |
        synth_clip2 |
        (synth_clip + synth_clip2)
    ).padded(0.1, 0).play()

#look_synth(); exit()


# pseudo-harmonics experiment
# hmm...
def expt_ph():

    #synth = synth_lib("synth_clarinet")
    synth = synth_lib("synth_guitar")

    duration = 3

    freq1 = 220
    freq2_just = 5/4 * freq1
    freq2_12tet = p2f(4) * freq1

    clip1 = synth(freq1, duration)
    clip1_ph = synth(freq1, duration, ph=True)
    clip2_12tet = synth(freq2_12tet, duration)
    clip2_12tet_ph = synth(freq2_12tet, duration, ph=True)
    clip2_just = synth(freq2_just, duration)

    (
        clip1 |
        clip1_ph |
        (clip1 + clip2_12tet) |
        (clip1_ph + clip2_12tet_ph) |
        (clip1 + clip2_just)
    ).padded(0.1, 0).play()
